...
public class TMP_TextSelector_B : MonoBehaviour, IPointerEnterHandler, IEventSystemHandler, IPointerExitHandler, IPointerClickHandler, IPointerUpHandler
{
	...
	private void LateUpdate()
	{
		if (isHoveringObject)
		{
			int num = TMP_TextUtilities.FindIntersectingCharacter((TMP_Text)m_TextMeshPro, Input.mousePosition, m_Camera, true);
			...
			int num3 = TMP_TextUtilities.FindIntersectingWord((TMP_Text)m_TextMeshPro, Input.mousePosition, m_Camera);
			...
			int num6 = TMP_TextUtilities.FindIntersectingLink((TMP_Text)m_TextMeshPro, Input.mousePosition, m_Camera);
			...
			if (!(linkID == "id_01"))
			{
				if (linkID == "id_02")
				{
					((Transform)m_TextPopup_RectTransform).position = worldPoint;
